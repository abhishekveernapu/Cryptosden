  ...
  const handleOAuthLogin = async (provider, providerName) => {
    try {
      ...
      if (err.code === "auth/account-exists-with-different-credential") {
        ...
        setError(err.response?.data?.message || `Failed to login with ${providerName}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="container" style={{ padding: "40px 0" }}>
      <div className="card" style={{ maxWidth: "420px", margin: "0 auto" }}>
        ...
        {error && (
          <div style={{ marginBottom: "12px", fontSize: "13px", color: "var(--red)" }}>
            {error}
          </div>
        )}
        ...
        <div style={{ display: "flex", alignItems: "center", margin: "20px 0" }}>
          ...
          <span style={{ padding: "0 10px", fontSize: "12px", color: "var(--text-secondary)" }}>OR</span>
          <div style={{ flex: 1, height: "1px", background: "var(--border)" }}></div>
        </div>
